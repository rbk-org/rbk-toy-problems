...
*/
 ...
 We can think of 5 as the upper bound of the guess. //true
 How many times is the upper bound repeated? // 
 What if we wanted to change the upper bound to 6?  // in hhe randInt(6) , if(n>5) and return statment  "Out of bounds! Please try a number between 0 and 6"
...
function guessMyNumber1(n) { 
  var upperBound = 7;
  var expect= randInt(upperBound);
  if (n > upperBound) { 
    return 'Out of bounds! Please try a number between 0 and '+upperBound+'.'; 
  } else if (n === expect) { 
      return 'You guessed my number!'; 
  } 
      return "Nope! The correct number was: "+expect+""; 
 }
...
change the guessing game so that it picks a random number once and allows you to guess until you get the correct answer.
 ...
 var upperBound2 = 7;
 var expect2 = randInt(upperBound2);
 ...
 return "Nope! The correct number was: "+expect2+""; 
 } 
...
w=0
        function guessMyNumber(n) { 
            ...
            y=randInt(UpperBound)
            if(x===1){w=1;x=0;return 'You guessed my number in 1 guess, highscore = '+w}
            if(x<=5){if(x<w){w=x};r=x;x=0;return 'You guessed my number in '+r+' guesses, highscore = '+w}
            if(x>5){return 'You have exceeded the guesses limit'}
            } 
            x++
