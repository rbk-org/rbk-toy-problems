...
//     }
    ...
    Possible if we wanna too
    How many changes would be required?  
    three => we shall replace 5 by 6 in randInt().
    and then (n > 6) in if statment.
    At the end we change the return statement with the new upper 'Please try a number between 0 and 6.' not 5.
   ...
   */
   ...
   function guessMyNumber(n) { 
    ...
    while(n !== random  ){
        n = parseInt(window.prompt("Please try another num"))
        count++
        if(count > 11 ){
            upperBound--
            return " oOoOpPs ! you exceeded the limts"
        }
    }
    if(count < lowest){
        upperBound++
        lowest = count;
        console.log("new high score recorded " + lowest ) 
    }
