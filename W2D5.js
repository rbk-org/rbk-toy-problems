...
function guessMyNumber(n,upperBound) { 
 ...
 return 'guessMyNumber includes the correct guess in its output'; 
 } 
 return 'Nope! The correct number was:'+ randInt(n); 
 } 
 function randInt(n) { 
 return Math.floor(Math.random() * (n + 1)) 
 }
...
function guessMyNumber(n) { 
  var upperBound=5; var f=-1 ; var c;var count;
  if (f===-1){
    c =randInt(upperBound);
    count =1;
f= 1;
    }
count ++;
 ...
 if (count ===1){
   ...
   else if (count ===5){ return 'you have exceded the limit ';}

 return 'you have guessed the num in ' + count + ' times'; 
 } 
  else{f=-1;
     return 'Nope!'; }
 } 
