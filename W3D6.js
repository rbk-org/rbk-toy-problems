...
The list is mentioned to be in "decreasing priority" simply because you do not have to sort the input array to optimize 
for anything else.
...
function shoppingSummary(array){
    ...
    list.add : function (item){
      if(cost-array[i].price >= 0 ){
        cost -= array[i].price
      
      for (var i =0 ;i < array.length ; i++){
      if (array[i].item == item)
      listArr.push(array[i])
        if(cost-array[i].price >= 0 )
        cost -= array[i].price
         }
       }
...
function removeMostExpensive (array){
 highPrice = array[0].price
	for (var i = 1; i <= array; i++ )
		if(array[i].price > highPrice){
      highPrice =array[i].price
    }
	}
  array.splice ()
}